          ...
          <div key={index} className="md:col-span-3">
            <div>
              <Card
                ...
                cover={<Image alt={item?.recipe?.label} src={item?.recipe?.image} />}
              >
                <div className="h-40">
                  <div className="h-30" onClick={() => handleDetails(item?.recipe)}>
                    <div className="font-extrabold">{item?.recipe?.label}</div>
                    <div className="font-bold">Calories: {item?.recipe?.calories.toFixed(3)}</div>
                  </div>
                  <div className="">
